...
class GameScene {
  constructor(game) {
    ...
    // 触摸手势相关
    this.touches = []; // 当前触摸点数组
    this.isPanning = false; // 是否正在拖动地图
    this.panStartX = 0;
    this.panStartY = 0;
    this.panOffsetX = 0; // 地图拖动偏移X
    this.panOffsetY = 0; // 地图拖动偏移Y
  ...
  calculateRenderParams() {
    ...
    this.offsetY = 80 + (gameAreaHeight - totalHeight) / 2 + this.panOffsetY;
  }
  ...
  handleTouchStart(touches) {
    ...
    if (this.touches.length === 1) {
      ...
      if (this.checkSliderClick(touch.x, touch.y)) {
        ...
        this.sliderStartX = touch.x;
        return;
      }
      
      // 记录起始位置，但不立即开始拖动（等待移动事件）
      this.isPanning = false; // 初始状态不是拖动，等待移动事件确认
  ...
  handleTouchEnd(touches) {
    ...
    if (remainingTouches.length === 0) {
      ...
      // 从双指变为单指，切换到拖动模式
      this.isPanning = false; // 重置，等待移动事件确认
  ...
  checkSliderClick(x, y) {
    ...
    // 检查是否点击了减号按钮
    const minusX = sliderX - sliderWidth / 2 - 30;
    ...
    // 检查是否点击了加号按钮
    const plusX = sliderX + sliderWidth / 2 + 30;
    ...
    const sliderTop = sliderY - Math.max(handleSize / 2, 15);
    const sliderBottom = sliderY + Math.max(handleSize / 2, 15);
