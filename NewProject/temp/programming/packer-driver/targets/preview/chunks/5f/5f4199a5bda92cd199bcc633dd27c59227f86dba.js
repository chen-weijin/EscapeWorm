    ...
    execute: function () {
      ...
      _export("GameController", GameController = (_dec = ccclass('GameController'), _dec2 = property(Node), _dec3 = property(Node), _dec4 = property(Label), _dec5 = property([Node]), _dec6 = property(Node), _dec(_class = (_class2 = class GameController extends Component {
        ...
        calculateRenderParams() {
          ...
          var areaHeight = transform.height; // 计算基础 cellSize（不考虑缩放）

          var baseCellSizeX = areaWidth / this.matrix.width;
          var baseCellSizeY = areaHeight / this.matrix.height;
          var baseCellSize = Math.min(baseCellSizeX, baseCellSizeY) * 0.9; // 应用缩放比例
          ...
          var centerOffsetX = (areaWidth - gridWidth) / 2 - areaWidth / 2;
          var centerOffsetY = (areaHeight - gridHeight) / 2 - areaHeight / 2;
        ...
        isValidEscapePosition(pos) {
          ...
          var matrixWidth = this.matrix.width;
          var matrixHeight = this.matrix.height;
          var maxDistance = 2; // 允许的最大距离（从边界算起）
          // 检查是否在矩阵边界外
          ...
          var xInRange = pos.x >= -maxDistance && pos.x < matrixWidth + maxDistance;
          var yInRange = pos.y >= -maxDistance && pos.y < matrixHeight + maxDistance;
        ...
        animateWormsAppearance() {
          ...
          return _asyncToGenerator(function* () {
            ...
            var segmentInterval = 50; // 安全检查：确保 worms 是数组

            if (!Array.isArray(_this3.worms) || _this3.worms.length === 0) {
              (_crd && Logger === void 0 ? (_reportPossibleCrUseOfLogger({
                error: Error()
            ...
            for (var segmentIndex = 1; segmentIndex <= maxSegments; segmentIndex++) {
              for (var _worm of _this3.worms) {
                if (_worm && _worm.segments && segmentIndex <= _worm.segments.length) {
        ...
        updateWormVisuals() {
          ...
          for (var i = 0; i < this.worms.length; i++) {
            ...
            var wormNode = this.wormNodes[i]; // 安全检查
        ...
        onTouchMove(event) {
          ...
          var location = event.getUILocation();
          var deltaX = location.x - this.dragStartPos.x;
          var deltaY = location.y - this.dragStartPos.y; // 如果移动距离超过阈值，认为是拖动

          var dragThreshold = 10; // 拖动阈值（像素）
        ...
        limitDragRange() {
          ...
          if (!transform) return;
          var areaWidth = transform.width;
          var areaHeight = transform.height; // 计算网格的实际尺寸

          var gridWidth = this.matrix.width * this.cellSize;
          var gridHeight = this.matrix.height * this.cellSize; // 限制拖动范围，确保地图不会移出屏幕太远

          var maxOffsetX = Math.max(0, (gridWidth - areaWidth) / 2);
          var maxOffsetY = Math.max(0, (gridHeight - areaHeight) / 2);
          ...
          return _asyncToGenerator(function* () {
            try {
              ...
              var _loop = function* _loop() {
                ...
                var isOutOfBounds = _this5.isValidEscapePosition(nextPos); // 检查是否在逃脱点上（逃脱点必须在边界外才有效）
                ...
                if (isOutOfBounds) {
                  ...
                  var maxExtraSteps = Math.min(worm.getLength(), 10); // 最多额外移动10步或蠕虫长度

                  for (var extraStep = stepIndex + 1; extraStep < path.length && extraStep - stepIndex <= maxExtraSteps; extraStep++) {
                    var extraPos = path[extraStep]; // 检查额外位置是否在合理范围内（不超过边界太远）

                    if (_this5.isValidEscapePosition(extraPos)) {
                      (_crd && Logger === void 0 ? (_reportPossibleCrUseOfLogger({
                        error: Error()
              ...
              var isFinalOutOfBounds = _this5.isValidEscapePosition(finalPos);
        ...
        tryMoveWormOneStep(worm) {
          ...
          return _asyncToGenerator(function* () {
            ...
            var isOutOfBounds = _this7.isValidEscapePosition(nextPos); // 检查是否在逃脱点上（逃脱点必须在边界外才有效）
...
//# sourceMappingURL=5f4199a5bda92cd199bcc633dd27c59227f86dba.js.map