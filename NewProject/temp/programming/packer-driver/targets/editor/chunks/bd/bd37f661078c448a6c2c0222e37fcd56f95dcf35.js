  ...
  return {
    ...
    execute: function () {
      ...
      _export("Worm", Worm = (_dec = ccclass('Worm'), _dec(_class = (_class2 = class Worm extends Component {
        ...
        startMoveAnimation(newHeadPosition, duration = 40) {
          return new Promise((resolve, reject) => {
            try {
              ...
              if (this.isEscaped) {
                console.log(`蠕虫 ${this.id} 已逃脱，跳过动画`);
                resolve();
                return;
              }

              console.log(`蠕虫 ${this.id} 开始移动动画: 从 (${(_this$segments$ = this.segments[0]) == null ? void 0 : _this$segments$.x}, ${(_this$segments$2 = this.segments[0]) == null ? void 0 : _this$segments$2.y}) 到 (${newHeadPosition.x}, ${newHeadPosition.y}), 持续时间=${duration}ms`); // 保存当前位置作为动画起始位置
              ...
              setTimeout(() => {
                try {
                  console.log(`蠕虫 ${this.id} 动画完成回调执行`);
                  this.completeAnimation();
                  console.log(`蠕虫 ${this.id} 动画完成，resolve`);
                  resolve();
                } catch (error) {
                  console.error(`蠕虫 ${this.id} 动画完成回调中发生错误:`, error);
                  reject(error);
                }
              }, duration);
            } catch (error) {
              console.error(`蠕虫 ${this.id} startMoveAnimation 中发生错误:`, error);
              reject(error);
            }
          });
        }
        ...
        reset() {
          var _this$segments$3, _this$segments$4;
          ...
          console.log(`蠕虫 ${this.id} reset完成: segments=${this.segments.length}, visibleCount=${this.visibleSegmentCount}, 头部=(${(_this$segments$3 = this.segments[0]) == null ? void 0 : _this$segments$3.x}, ${(_this$segments$4 = this.segments[0]) == null ? void 0 : _this$segments$4.y})`);
        }
        /**
         * 设置可见段数量
         */
...
//# sourceMappingURL=bd37f661078c448a6c2c0222e37fcd56f95dcf35.js.map