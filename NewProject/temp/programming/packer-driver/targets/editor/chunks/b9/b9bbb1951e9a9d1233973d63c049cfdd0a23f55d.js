      ...
      _export("GameController", GameController = (_dec = ccclass('GameController'), _dec2 = property(Node), _dec3 = property(Node), _dec4 = property(Label), _dec5 = property([Node]), _dec(_class = (_class2 = class GameController extends Component {
          ...
          try {
            ...
            for (let stepIndex = 0; stepIndex < path.length; stepIndex++) {
              ...
              const isOutOfBounds = !(_crd && CollisionDetector === void 0 ? (_reportPossibleCrUseOfCollisionDetector({
                error: Error()
              }), CollisionDetector) : CollisionDetector).isInBounds(nextPos, this.matrix.width, this.matrix.height); // 检查是否在逃脱点上

              const isOnEscapePoint = this.escapePoints.some(ep => ep.x === nextPos.x && ep.y === nextPos.y);
              console.log(`蠕虫 ${worm.id} 步骤 ${stepIndex + 1}: 边界外=${isOutOfBounds}, 逃脱点=${isOnEscapePoint}`); // 如果已经在边界外或逃脱点上，立即标记为逃脱

              if (isOutOfBounds || isOnEscapePoint) {
            ...
            const isFinalOnEscapePoint = this.escapePoints.some(ep => ep.x === finalPos.x && ep.y === finalPos.y);
            console.log(`蠕虫 ${worm.id} 路径走完: 最终位置=(${finalPos.x}, ${finalPos.y}), 边界外=${isFinalOutOfBounds}, 逃脱点=${isFinalOnEscapePoint}`);

            if (isFinalOutOfBounds || isFinalOnEscapePoint) {
        ...
        async tryMoveWormOneStep(worm) {
          ...
          if (isOutOfBounds || isOnEscapePoint) {
            ...
            // 可以逃脱
            console.log(`边界外=${isOutOfBounds}, 逃脱点=${isOnEscapePoint}, 蠕虫逃脱`);
...
//# sourceMappingURL=b9bbb1951e9a9d1233973d63c049cfdd0a23f55d.js.map