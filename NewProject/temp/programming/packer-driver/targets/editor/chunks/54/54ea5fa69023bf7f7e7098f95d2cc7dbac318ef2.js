    ...
    execute: function () {
      ...
      _export("GameController", GameController = (_dec = ccclass('GameController'), _dec2 = property(Node), _dec3 = property(Node), _dec4 = property(Label), _dec5 = property([Node]), _dec6 = property(Node), _dec(_class = (_class2 = class GameController extends Component {
        ...
        calculateRenderParams() {
          ...
          (_crd && Logger === void 0 ? (_reportPossibleCrUseOfLogger({
            error: Error()
          }), Logger) : Logger).log('渲染参数: areaSize=', areaWidth, 'x', areaHeight, ' cellSize=', this.cellSize, ' offset=', this.offsetX, this.offsetY);
        }
        /**
         * 绘制网格
         */
          ...
          try {
            ...
            for (let stepIndex = 0; stepIndex < path.length; stepIndex++) {
              ...
              const isOutOfBounds = !(_crd && CollisionDetector === void 0 ? (_reportPossibleCrUseOfCollisionDetector({
                error: Error()
              }), CollisionDetector) : CollisionDetector).isInBounds(nextPos, this.matrix.width, this.matrix.height); // 检查是否在逃脱点上（逃脱点必须在边界外才有效）

              const isOnEscapePoint = isOutOfBounds && this.escapePoints.some(ep => ep.x === nextPos.x && ep.y === nextPos.y);
              (_crd && Logger === void 0 ? (_reportPossibleCrUseOfLogger({
                error: Error()
              }), Logger) : Logger).log(`蠕虫 ${worm.id} 步骤 ${stepIndex + 1}: 边界外=${isOutOfBounds}, 逃脱点=${isOnEscapePoint}, 位置=(${nextPos.x}, ${nextPos.y})`); // 只有真正在边界外时才能逃脱
              ...
              if (isOutOfBounds) {
                ...
                for (let extraStep = stepIndex + 1; extraStep < path.length; extraStep++) {
            ...
            const isFinalOutOfBounds = !(_crd && CollisionDetector === void 0 ? (_reportPossibleCrUseOfCollisionDetector({
              error: Error()
            }), CollisionDetector) : CollisionDetector).isInBounds(finalPos, this.matrix.width, this.matrix.height);
            const isFinalOnEscapePoint = isFinalOutOfBounds && this.escapePoints.some(ep => ep.x === finalPos.x && ep.y === finalPos.y);
            (_crd && Logger === void 0 ? (_reportPossibleCrUseOfLogger({
              error: Error()
            }), Logger) : Logger).log(`蠕虫 ${worm.id} 路径走完: 最终位置=(${finalPos.x}, ${finalPos.y}), 边界外=${isFinalOutOfBounds}, 逃脱点=${isFinalOnEscapePoint}`); // 只有真正在边界外时才能逃脱
        ...
        async tryMoveWormOneStep(worm) {
          ...
          if (isOutOfBounds) {
            ...
            (_crd && Logger === void 0 ? (_reportPossibleCrUseOfLogger({
              error: Error()
            }), Logger) : Logger).log(`边界外=${isOutOfBounds}, 逃脱点=${isOnEscapePoint}, 位置=(${nextPos.x}, ${nextPos.y}), 蠕虫逃脱`);
...
//# sourceMappingURL=54ea5fa69023bf7f7e7098f95d2cc7dbac318ef2.js.map