System.register(["__unresolved_0", "cc", "__unresolved_1", "__unresolved_2", "__unresolved_3", "__unresolved_4", "__unresolved_5", "__unresolved_6", "__unresolved_7", "__unresolved_8"], function (_export, _context) {
  ...
  var _reporterNs, _cclegacy, __checkObsolete__, __checkObsoleteInNamespace__, _decorator, Component, Node, director, Vec2, Vec3, UITransform, Graphics, Color, Label, Sprite, SpriteFrame, resources, Input, Worm, PathFinder, CollisionDetector, LevelManager, StorageManager, AudioManager, EffectManager, Logger, _dec, _dec2, _dec3, _dec4, _dec5, _dec6, _class, _class2, _descriptor, _descriptor2, _descriptor3, _descriptor4, _descriptor5, _descriptor6, _crd, ccclass, property, GameController;
  ...
  return {
    ...
    execute: function () {
      ...
      _export("GameController", GameController = (_dec = ccclass('GameController'), _dec2 = property(Node), _dec3 = property(Node), _dec4 = property(Label), _dec5 = property([Node]), _dec6 = property(Node), _dec(_class = (_class2 = class GameController extends Component {
        constructor(...args) {
          super(...args);
          ...
          _initializerDefineProperty(this, "gameSceneController", _descriptor5, this);
        ...
        setupGameArea() {
          ...
          if (uiTransform.width === 0 || uiTransform.height === 0) {
            ...
            (_crd && Logger === void 0 ? (_reportPossibleCrUseOfLogger({
              error: Error()
            }), Logger) : Logger).log('设置 gameArea 默认尺寸:', uiTransform.width, 'x', uiTransform.height);
          ...
          (_crd && Logger === void 0 ? (_reportPossibleCrUseOfLogger({
            error: Error()
          }), Logger) : Logger).log('触摸事件已注册到 gameArea, size=', uiTransform.width, 'x', uiTransform.height);
        ...
        async loadWormImages() {
          try {
            ...
            (_crd && Logger === void 0 ? (_reportPossibleCrUseOfLogger({
              error: Error()
            }), Logger) : Logger).warn('蠕虫图片资源加载失败，将使用默认绘制方式');
          }
        }
        /**
         * 加载 SpriteFrame
        ...
        async initLevel(levelId) {
          ...
          try {
            ...
            for (const wormConfig of levelData.worms) {
              ...
              (_crd && Logger === void 0 ? (_reportPossibleCrUseOfLogger({
                error: Error()
              }), Logger) : Logger).log(`蠕虫 ${worm.id} 创建: 头部=(${worm.getHeadPosition().x}, ${worm.getHeadPosition().y}), 方向=${worm.direction}, 段数=${worm.segments.length}`);
            } // 绘制网格
            ...
            (_crd && Logger === void 0 ? (_reportPossibleCrUseOfLogger({
              error: Error()
            }), Logger) : Logger).error('初始化游戏场景失败:', error);
            throw error;
          }
        }
        /**
        ...
        calculateRenderParams() {
          ...
          const areaHeight = transform.height; // 计算基础 cellSize（不考虑缩放）

          const baseCellSizeX = areaWidth / this.matrix.width;
          const baseCellSizeY = areaHeight / this.matrix.height;
          const baseCellSize = Math.min(baseCellSizeX, baseCellSizeY) * 0.9; // 应用缩放比例

          this.cellSize = baseCellSize * this.zoomScale; // 计算偏移量使网格居中，并加上拖动偏移
          ...
          (_crd && Logger === void 0 ? (_reportPossibleCrUseOfLogger({
            error: Error()
          }), Logger) : Logger).log('渲染参数: areaSize=', areaWidth, 'x', areaHeight, ' cellSize=', this.cellSize, ' zoomScale=', this.zoomScale, ' offset=', this.offsetX, this.offsetY);
        }
        /**
         * 设置缩放比例
        ...
        setZoomScale(scale) {
          // 限制缩放范围
          this.zoomScale = Math.max(0.5, Math.min(1.5, scale)); // 重新计算渲染参数

          this.calculateRenderParams(); // 限制拖动范围（缩放后可能需要调整）
        ...
        isValidEscapePosition(pos) {
          ...
          const isOutOfBounds = !(_crd && CollisionDetector === void 0 ? (_reportPossibleCrUseOfCollisionDetector({
            error: Error()
          }), CollisionDetector) : CollisionDetector).isInBounds(pos, matrixWidth, matrixHeight);
        ...
        async animateWormsAppearance() {
          ...
          const segmentInterval = 50; // 安全检查：确保 worms 是数组

          if (!Array.isArray(this.worms) || this.worms.length === 0) {
            (_crd && Logger === void 0 ? (_reportPossibleCrUseOfLogger({
              error: Error()
            }), Logger) : Logger).warn('animateWormsAppearance: worms 不是数组或为空');
            this.isWormsAppearing = false;
            return;
          }
        ...
        updateWormVisuals() {
          ...
          for (let i = 0; i < this.worms.length; i++) {
            ...
            const wormNode = this.wormNodes[i]; // 安全检查
            ...
            if (!wormNode.active) {
              (_crd && Logger === void 0 ? (_reportPossibleCrUseOfLogger({
                error: Error()
              }), Logger) : Logger).log(`激活蠕虫节点 ${worm.id}`);
            ...
            if (!segments || segments.length === 0) {
              (_crd && Logger === void 0 ? (_reportPossibleCrUseOfLogger({
                error: Error()
              }), Logger) : Logger).error(`错误：蠕虫 ${worm.id} 的 segments 为空！`);
              continue;
            }
            ...
            if (visibleCount === 0 && !worm.hasEscaped()) {
              (_crd && Logger === void 0 ? (_reportPossibleCrUseOfLogger({
                error: Error()
              }), Logger) : Logger).warn(`警告：蠕虫 ${worm.id} 的 visibleCount=0，但未逃脱！`);
            } // 更新每个段的位置
        ...
        logRenderState(worm) {
          ...
          (_crd && Logger === void 0 ? (_reportPossibleCrUseOfLogger({
            error: Error()
          }), Logger) : Logger).log(`渲染蠕虫 ${worm.id}: segments=${segments.length}, visible=${worm.visibleSegmentCount}, head=(${head.x}, ${head.y}), screen=(${screenPos.x.toFixed(0)}, ${screenPos.y.toFixed(0)}), animating=${worm.isAnimating}, cellSize=${this.cellSize.toFixed(1)}, offset=(${this.offsetX.toFixed(0)}, ${this.offsetY.toFixed(0)})`);
        }
        /**
         * 调试：打印蠕虫状态
         */
        ...
        debugWormState() {
          (_crd && Logger === void 0 ? (_reportPossibleCrUseOfLogger({
            error: Error()
          }), Logger) : Logger).log('=== 蠕虫状态 ===');

          if (!Array.isArray(this.worms)) {
            (_crd && Logger === void 0 ? (_reportPossibleCrUseOfLogger({
              error: Error()
            }), Logger) : Logger).log('worms 不是数组');
            return;
          }
          ...
          for (const worm of this.worms) {
            (_crd && Logger === void 0 ? (_reportPossibleCrUseOfLogger({
              error: Error()
            }), Logger) : Logger).log(`蠕虫 ${worm.id}: escaped=${worm.isEscaped}, escaping=${worm.isEscaping}, animating=${worm.isAnimating}, segments=${worm.segments.length}, visible=${worm.visibleSegmentCount}`);
          }
        }
        /**
         * 渲染蠕虫段
        ...
        renderWormSegments(wormNode, worm, segments, visibleCount) {
          ...
          for (let i = 0; i < segments.length; i++) {
            ...
            if (i === 0 && !isVisible && !worm.hasEscaped()) {// Logger.warn(`警告：蠕虫 ${worm.id} 头部被隐藏！segments.length=${segments.length}, visibleCount=${visibleCount}, isVisible=${isVisible}`);
            }
            ...
            if (i === 0) {
              ...
              if (transform) {
                ...
                if (Math.abs(screenPos.x) > halfWidth * 2 || Math.abs(screenPos.y) > halfHeight * 2) {//Logger.warn(`警告：蠕虫 ${worm.id} 头部坐标超出屏幕范围！world=(${segment.x}, ${segment.y}) screen=(${screenPos.x.toFixed(0)}, ${screenPos.y.toFixed(0)}), 屏幕范围=(${halfWidth}, ${halfHeight})`);
                }
              }
            } // 绘制段
        ...
        onTouchEnd(event) {
          ...
          if (this.isGameOver || this.isVictory || this.isWormsAppearing) {
            (_crd && Logger === void 0 ? (_reportPossibleCrUseOfLogger({
              error: Error()
            }), Logger) : Logger).log('触摸被忽略: isGameOver=', this.isGameOver, 'isVictory=', this.isVictory, 'isWormsAppearing=', this.isWormsAppearing);
            return;
          } // 如果是拖动，不处理点击
          ...
          if (!transform) {
            (_crd && Logger === void 0 ? (_reportPossibleCrUseOfLogger({
              error: Error()
            }), Logger) : Logger).log('transform 不存在');
            return;
          } // 转换为节点本地坐标（考虑地图偏移）
          ...
          (_crd && Logger === void 0 ? (_reportPossibleCrUseOfLogger({
            error: Error()
          }), Logger) : Logger).log('触摸位置: UI=', location.x, location.y, ' Local=', localPos.x, localPos.y); // 查找点击的蠕虫
          ...
          if (clickedWorm) {
            (_crd && Logger === void 0 ? (_reportPossibleCrUseOfLogger({
              error: Error()
            }), Logger) : Logger).log('找到蠕虫:', clickedWorm.id, 'escaped=', clickedWorm.hasEscaped(), 'animating=', clickedWorm.isAnimating);
        ...
        limitDragRange() {
          ...
          if (!transform) return;
          const areaWidth = transform.width;
          const areaHeight = transform.height; // 计算网格的实际尺寸

          const gridWidth = this.matrix.width * this.cellSize;
          const gridHeight = this.matrix.height * this.cellSize; // 限制拖动范围，确保地图不会移出屏幕太远
        ...
        async handleWormClick(worm) {
          ...
          (_crd && Logger === void 0 ? (_reportPossibleCrUseOfLogger({
            error: Error()
          }), Logger) : Logger).log('处理蠕虫点击: id=', worm.id, ' head=', worm.getHeadPosition(), ' dir=', worm.direction);
          ...
          (_crd && Logger === void 0 ? (_reportPossibleCrUseOfLogger({
            error: Error()
          }), Logger) : Logger).log('障碍物数量:', obstacles.length); // 查找逃脱路径
          ...
          (_crd && Logger === void 0 ? (_reportPossibleCrUseOfLogger({
            error: Error()
          }), Logger) : Logger).log('路径查找结果: canEscape=', result.canEscape, ' pathLen=', result.path.length);

          try {
            if (result.canEscape && result.path.length > 0) {
              (_crd && Logger === void 0 ? (_reportPossibleCrUseOfLogger({
                error: Error()
              }), Logger) : Logger).log('蠕虫开始逃脱');
              await this.moveWormToEscape(worm, result.path);
              (_crd && Logger === void 0 ? (_reportPossibleCrUseOfLogger({
                error: Error()
              }), Logger) : Logger).log('蠕虫逃脱流程完成');
            } else if (!result.canEscape && result.pathToObstacle && result.pathToObstacle.length > 0) {
              (_crd && Logger === void 0 ? (_reportPossibleCrUseOfLogger({
                error: Error()
              }), Logger) : Logger).log('蠕虫碰到障碍物，后退');
              await this.moveWormToObstacleAndBack(worm, result.pathToObstacle);
              (_crd && Logger === void 0 ? (_reportPossibleCrUseOfLogger({
                error: Error()
              }), Logger) : Logger).log('蠕虫后退流程完成');
            } else {
              (_crd && Logger === void 0 ? (_reportPossibleCrUseOfLogger({
                error: Error()
              }), Logger) : Logger).log('尝试向前移动一步');
              await this.tryMoveWormOneStep(worm);
              (_crd && Logger === void 0 ? (_reportPossibleCrUseOfLogger({
                error: Error()
              }), Logger) : Logger).log('移动一步流程完成');
            }
          } catch (error) {
            (_crd && Logger === void 0 ? (_reportPossibleCrUseOfLogger({
              error: Error()
            }), Logger) : Logger).error('处理蠕虫移动时发生错误:', error);
            (_crd && Logger === void 0 ? (_reportPossibleCrUseOfLogger({
              error: Error()
            }), Logger) : Logger).error('错误堆栈:', error.stack);
          }
        }
        /**
         * 获取障碍物
        ...
        async moveWormToEscape(worm, path) {
          try {
            (_crd && Logger === void 0 ? (_reportPossibleCrUseOfLogger({
              error: Error()
            }), Logger) : Logger).log(`蠕虫 ${worm.id} 开始逃脱，路径长度=${path.length}`);
            ...
            for (let stepIndex = 0; stepIndex < path.length; stepIndex++) {
              ...
              (_crd && Logger === void 0 ? (_reportPossibleCrUseOfLogger({
                error: Error()
              }), Logger) : Logger).log(`蠕虫 ${worm.id} 步骤 ${stepIndex + 1}/${path.length}: 移动到 (${nextPos.x}, ${nextPos.y})`); // 检查是否在边界外（可以逃脱）
              // 边界外应该是紧邻矩阵边界的位置，不应该太远

              const isOutOfBounds = this.isValidEscapePosition(nextPos); // 检查是否在逃脱点上（逃脱点必须在边界外才有效）
              ...
              if (isOutOfBounds) {
                var _this$audioManager2, _this$audioManager3, _this$effectManager;

                (_crd && Logger === void 0 ? (_reportPossibleCrUseOfLogger({
                  error: Error()
                }), Logger) : Logger).log(`蠕虫 ${worm.id} 在步骤 ${stepIndex + 1} 到达逃脱位置，开始逃脱`);
                await worm.startMoveAnimation(nextPos, moveDuration);
                (_this$audioManager2 = this.audioManager) == null ? void 0 : _this$audioManager2.playSound('move'); // 继续移动剩余步数，让整个蠕虫都离开屏幕
                // 但限制额外步数，避免移动到过远的位置
                ...
                for (let extraStep = stepIndex + 1; extraStep < path.length && extraStep - stepIndex <= maxExtraSteps; extraStep++) {
                  const extraPos = path[extraStep]; // 检查额外位置是否在合理范围内（不超过边界太远）

                  if (this.isValidEscapePosition(extraPos)) {
                    (_crd && Logger === void 0 ? (_reportPossibleCrUseOfLogger({
                      error: Error()
                    }), Logger) : Logger).log(`蠕虫 ${worm.id} 额外步骤: 移动到 (${extraPos.x}, ${extraPos.y})`);
                ...
                (_crd && Logger === void 0 ? (_reportPossibleCrUseOfLogger({
                  error: Error()
                }), Logger) : Logger).log(`蠕虫 ${worm.id} 逃脱完成`);
              ...
              if (!Array.isArray(this.worms)) {
                (_crd && Logger === void 0 ? (_reportPossibleCrUseOfLogger({
                  error: Error()
                }), Logger) : Logger).error('worms 不是数组，无法检查碰撞');
                return;
              }
              ...
              (_crd && Logger === void 0 ? (_reportPossibleCrUseOfLogger({
                error: Error()
              }), Logger) : Logger).log(`蠕虫 ${worm.id} 步骤 ${stepIndex + 1}: 碰撞检测结果=${hasCollision}`);

              if (hasCollision) {
                (_crd && Logger === void 0 ? (_reportPossibleCrUseOfLogger({
                  error: Error()
                }), Logger) : Logger).log(`蠕虫 ${worm.id} 在步骤 ${stepIndex + 1} 发生碰撞，调用 handleCollision`);
                await this.handleCollision(worm);
                (_crd && Logger === void 0 ? (_reportPossibleCrUseOfLogger({
                  error: Error()
                }), Logger) : Logger).log(`蠕虫 ${worm.id} handleCollision 完成`);
                return;
              }

              (_crd && Logger === void 0 ? (_reportPossibleCrUseOfLogger({
                error: Error()
              }), Logger) : Logger).log(`蠕虫 ${worm.id} 步骤 ${stepIndex + 1}: 开始移动动画`);
              await worm.startMoveAnimation(nextPos, moveDuration);
              (_crd && Logger === void 0 ? (_reportPossibleCrUseOfLogger({
                error: Error()
              }), Logger) : Logger).log(`蠕虫 ${worm.id} 步骤 ${stepIndex + 1}: 移动动画完成`);
              (_this$audioManager4 = this.audioManager) == null ? void 0 : _this$audioManager4.playSound('move');
            ...
            if (isFinalOutOfBounds) {
              var _this$audioManager5, _this$effectManager2;
              ...
              (_this$effectManager2 = this.effectManager) == null ? void 0 : _this$effectManager2.createEscapeEffect(this.worldToScreen(worm.getHeadPosition().x, worm.getHeadPosition().y), worm.color);
              this.checkVictory();
            } else {
              (_crd && Logger === void 0 ? (_reportPossibleCrUseOfLogger({
                error: Error()
              }), Logger) : Logger).warn(`蠕虫 ${worm.id} 路径走完但仍在矩阵内，不应该逃脱！`); // 不应该逃脱，重置蠕虫
            ...
            (_crd && Logger === void 0 ? (_reportPossibleCrUseOfLogger({
              error: Error()
            }), Logger) : Logger).error(`蠕虫 ${worm.id} 逃脱过程中发生错误:`, error);
            (_crd && Logger === void 0 ? (_reportPossibleCrUseOfLogger({
              error: Error()
            }), Logger) : Logger).error('错误堆栈:', error.stack);
            worm.isEscaping = false;
        ...
        async moveWormToObstacleAndBack(worm, pathToObstacle) {
          var _this$audioManager6, _worm$originalSegment, _worm$originalSegment2;

          (_crd && Logger === void 0 ? (_reportPossibleCrUseOfLogger({
            error: Error()
          }), Logger) : Logger).log(`蠕虫 ${worm.id} 移动到障碍物: pathLen=${pathToObstacle.length}`);
          ...
          (_this$audioManager6 = this.audioManager) == null ? void 0 : _this$audioManager6.playSound('collision'); // 高亮闪烁

          await this.flashWorm(worm); // 返回原位

          (_crd && Logger === void 0 ? (_reportPossibleCrUseOfLogger({
            error: Error()
          }), Logger) : Logger).log(`蠕虫 ${worm.id} 重置到原位`);
          (_crd && Logger === void 0 ? (_reportPossibleCrUseOfLogger({
            error: Error()
          }), Logger) : Logger).log(`原始位置: originalSegments[0]=(${(_worm$originalSegment = worm.originalSegments[0]) == null ? void 0 : _worm$originalSegment.x}, ${(_worm$originalSegment2 = worm.originalSegments[0]) == null ? void 0 : _worm$originalSegment2.y}), length=${worm.originalSegments.length}`);
          worm.reset();
        ...
        async tryMoveWormOneStep(worm) {
          ...
          const isOnEscapePoint = isOutOfBounds && this.escapePoints.some(ep => ep.x === nextPos.x && ep.y === nextPos.y); // 只有真正在边界外时才能逃脱

          if (isOutOfBounds) {
            var _this$audioManager7, _this$effectManager3;

            // 可以逃脱
            console.log(`边界外=${isOutOfBounds}, 逃脱点=${isOnEscapePoint}, 位置=(${nextPos.x}, ${nextPos.y}), 蠕虫逃脱`);
            await worm.startMoveAnimation(nextPos, 80);
            worm.markEscaped();
            (_this$audioManager7 = this.audioManager) == null ? void 0 : _this$audioManager7.playSound('escape');
            (_this$effectManager3 = this.effectManager) == null ? void 0 : _this$effectManager3.createEscapeEffect(this.worldToScreen(nextPos.x, nextPos.y), worm.color);
            this.checkVictory();
            return;
          } // 检查碰撞
          ...
          if ((_crd && CollisionDetector === void 0 ? (_reportPossibleCrUseOfCollisionDetector({
            ...
            (_crd && Logger === void 0 ? (_reportPossibleCrUseOfLogger({
              error: Error()
            }), Logger) : Logger).log('正常移动一步到', nextPos.x, nextPos.y);
            await worm.startMoveAnimation(nextPos, 80);
            (_this$audioManager8 = this.audioManager) == null ? void 0 : _this$audioManager8.playSound('move');
          }
        }
        /**
         * 处理碰撞
        ...
        async handleCollision(worm) {
          var _this$audioManager9, _worm$originalSegment3, _worm$originalSegment4;

          (_crd && Logger === void 0 ? (_reportPossibleCrUseOfLogger({
            error: Error()
          }), Logger) : Logger).log(`碰撞处理开始: 蠕虫 ${worm.id}, 当前头部=(${worm.getHeadPosition().x}, ${worm.getHeadPosition().y})`);
          (_this$audioManager9 = this.audioManager) == null ? void 0 : _this$audioManager9.playSound('collision');
          await this.flashWorm(worm);
          (_crd && Logger === void 0 ? (_reportPossibleCrUseOfLogger({
            error: Error()
          }), Logger) : Logger).log(`闪烁完成，准备重置`);
          (_crd && Logger === void 0 ? (_reportPossibleCrUseOfLogger({
            error: Error()
          }), Logger) : Logger).log(`原始位置: originalSegments[0]=(${(_worm$originalSegment3 = worm.originalSegments[0]) == null ? void 0 : _worm$originalSegment3.x}, ${(_worm$originalSegment4 = worm.originalSegments[0]) == null ? void 0 : _worm$originalSegment4.y}), length=${worm.originalSegments.length}`);
          worm.reset();
          worm.isEscaping = false;
          (_crd && Logger === void 0 ? (_reportPossibleCrUseOfLogger({
            error: Error()
          }), Logger) : Logger).log(`重置完成: isEscaped=${worm.isEscaped}, segments=${worm.segments.length}, head=(${worm.getHeadPosition().x}, ${worm.getHeadPosition().y})`);
          this.logRenderState(worm); // 强制刷新渲染
        ...
        victory() {
          var _this$audioManager10, _this$storageManager;

          this.isVictory = true;
          this.isGameOver = true;
          (_this$audioManager10 = this.audioManager) == null ? void 0 : _this$audioManager10.playSound('victory');
        ...
        gameOver() {
          var _this$audioManager11;

          this.isGameOver = true;
          (_this$audioManager11 = this.audioManager) == null ? void 0 : _this$audioManager11.playSound('fail');
          this.scheduleOnce(() => {
        ...
        showResult(isVictory) {
          (_crd && Logger === void 0 ? (_reportPossibleCrUseOfLogger({
            error: Error()
          }), Logger) : Logger).log(isVictory ? '胜利！' : '失败！'); // 查找 GameSceneController 并调用其 showResult 方法
          ...
          if (sceneController && typeof sceneController.showResult === 'function') {
            ...
            (_crd && Logger === void 0 ? (_reportPossibleCrUseOfLogger({
              error: Error()
            }), Logger) : Logger).error('未找到 GameSceneController，无法显示结果');
          }
        }
        /**
         * 更新 UI
      ...
      }), _descriptor5 = _applyDecoratedDescriptor(_class2.prototype, "gameSceneController", [_dec6], {
        configurable: true,
        enumerable: true,
        writable: true,
        initializer: function () {
      ...
      }), _descriptor6 = _applyDecoratedDescriptor(_class2.prototype, "cellSize", [property], {
        configurable: true,
        enumerable: true,
        writable: true,
        initializer: function () {
...
//# sourceMappingURL=b1cf281cb83ea14e52d5f4d2ce13ffc7dee57710.js.map